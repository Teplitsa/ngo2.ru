...
/** Default filters **/
add_filter( 'ruka_the_content', 'wptexturize'        );
add_filter( 'ruka_the_content', 'convert_smilies'    );
add_filter( 'ruka_the_content', 'convert_chars'      );
add_filter( 'ruka_the_content', 'wpautop'            );
add_filter( 'ruka_the_content', 'shortcode_unautop'  );
add_filter( 'ruka_the_content', 'do_shortcode' );

add_filter( 'ruka_the_title', 'wptexturize'   );
add_filter( 'ruka_the_title', 'convert_chars' );
add_filter( 'ruka_the_title', 'trim'          );
...
/** more link */
function ruka_get_excerpt_with_link($cpost) {
	$more = '&gt;&gt;';
...
function ruka_continue_reading_link() {
	$more = '&gt;&gt;';
	return '&nbsp;<a href="'. esc_url( get_permalink() ) . '"><span class="meta-nav">'.$more.'</span></a>';
}

/** excerpt filters  */
add_filter( 'excerpt_more', 'ruka_auto_excerpt_more' );
function ruka_auto_excerpt_more( $more ) {
	return '&hellip;';
}

add_filter( 'excerpt_length', 'ruka_custom_excerpt_length' );
function ruka_custom_excerpt_length( $l ) {
	return 30;
}

/** inject */
add_filter( 'get_the_excerpt', 'ruka_custom_excerpt_more' );
function ruka_custom_excerpt_more( $output ) {
	global $post;
	...
	if($post->post_type == 'post')
		$output .= ruka_continue_reading_link();
 ...
 * Current URL
 **/
if(!function_exists('ruka_current_url')){
function ruka_current_url() {
...
function ruka_get_posts_ids_from_query($query){
...
/** Copyright string */
function ruka_dynamic_copyright($start_year, $name){
 ...
 **/
function ruka_print_top_link(){

	if(!is_admin()):
 ?>	
	<div id="top-link">
		<a href="#top"><?php _e('On top', 'ruka');?></a>
	</div>
	
<?php endif; 
}
add_action('wp_footer', 'ruka_print_top_link');


/**
 * Favicon
 **/
function ruka_favicon(){
...
add_action('wp_head', 'ruka_favicon', 1);
add_action('admin_head', 'ruka_favicon', 1);
add_action('login_head', 'ruka_favicon', 1);
 ...
 * @return array
 */
function ruka_body_classes( $classes ) {
	

	return $classes;
}
//add_filter( 'body_class', 'ruka_body_classes' );
 ...
 * 
 **/
add_filter( 'pre_wp_nav_menu', 'ruka_pre_wp_nav_menu_social', 10, 2 );
function ruka_pre_wp_nav_menu_social( $output, $args ) {
	...
	$supported_icons = apply_filters( 'ruka_supported_social_icons', array(
		'app.net'            => 'fa-adn',
...
function ruka_http_the_content($html){
