 ...
 **/
add_shortcode('ruka_query', 'ruka_query_screen');
function ruka_query_screen($atts){
	global $wp_query;
	...
	$css = esc_attr($css);
	return  "<div class='ruka-query {$css}'>{$out}</div>";
} 
...
function repeatable_blocks_screen($atts){
	...
	?>
		<section class="rb-item">
			...
			<?php if(!empty($title)): ?>
				<h3><?php echo apply_filters('ruka_the_title', $title);?></h3>
			<?php endif; ?>
 ...
 **/
add_shortcode('my_togle', 'ruka_my_toggle_screen');
function ruka_my_toggle_screen($atts, $content = ''){
	...
	?>
		<section class="page-section <?php echo esc_attr($css);?>" <?php echo $style;?>>
			...
			<?php if(!empty($title)): ?>
				...
				<?php echo apply_filters('ruka_the_title', $title);?>
				</h3>
			<?php endif; ?>
 ...
 * Markup shortcodes
 **/
add_shortcode('clear', 'ruka_clear_screen');
function ruka_clear_screen($atts){
...
add_shortcode('partners_gallery', 'ruka_partners_gallery_screen');
function ruka_partners_gallery_screen($atts){
...
?>
    ...
    <?php
		foreach($query->posts as $item):
			...
			$cat = ruka_get_partner_type($item);
        ?>
