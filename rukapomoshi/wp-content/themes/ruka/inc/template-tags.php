...
/* CPT Filters */
//add_action('parse_query', 'ruka_request_corrected');
function ruka_request_corrected($query) {
...
function is_programms() {
	
		
	if(is_page_branch('programms'))
		return true;
...
if ( ! function_exists( 'ruka_paging_nav' ) ) :
/**
 * Display paging nav
 */
function ruka_paging_nav($query = null) {
	global $wp_query;
...
?>
	<nav class="navigation paging-navigation" role="navigation">
		...
		<?php
			$p = ruka_paginate_links($query, false);
			if(!empty($p)):
		?>
			<div class="pagination"><?php ruka_paginate_links($query); ?></div>
		<?php endif; ?>
...
function ruka_paginate_links($query = null, $echo = true) {
...
if ( ! function_exists( 'ruka_post_nav' ) ) :
/**
 * Display navigation to next/previous post when applicable.
 */
function ruka_post_nav() {
	...
	<nav class="navigation post-navigation" role="navigation">
		<h1 class="screen-reader-text"><?php _e( 'Post navigation', 'ruka' ); ?></h1>
		<div class="nav-links">
			<?php
...
if ( ! function_exists( 'ruka_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function ruka_posted_on() {
	$pt = get_post_type();
	$cat = '';
 ...
 **/
function ruka_get_post_thumbnail($cpost = null, $size = 'post-thumbnail'){
	global $post;
...
function ruka_breadcrumbs(){
	...
	if(is_singular('post')) {
		...
		if($p){
			...
			if(!empty($cat)){
				$links[] = "<a href='".get_term_link($cat[0])."'>".apply_filters('ruka_the_title', $cat[0]->name)."</a>";
			}
		}	
	}
	elseif(is_singular('event')) {
		...
		if($p){
			...
			$pt_name = ruka_get_post_type_archive_title('event');
			if(!empty($pt_name)){
...
function ruka_get_post_type_archive_title($post_type) {
...
function ruka_event_meta($cpost = null) {
	global $post;
	...
	if(!empty($date)){
		...
		echo "<span class='label'>".__('Event date', 'ruka').":</span>";
		echo "<time class='e-date'>".date('d.m.Y', strtotime($date))."</time>";
		echo "</div>";
	}
	if(!empty($time)){
		echo "<div class='em-field'>";
		echo "<span class='label'>".__('Time', 'ruka').":</span>";
		echo "<time class='e-time'>".apply_filters('ruka_the_title', $time)."</time>";
		echo "</div>";
	}
	if(!empty($addr)){
		echo "<div class='em-field'>";
		echo "<span class='label'>".__('Address', 'ruka').":</span>";
		echo "<time class='e-time'>".apply_filters('ruka_the_title', $addr)."</time>";
		echo "</div>";
	}
