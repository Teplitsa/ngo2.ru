...
function step_post_nav() {
	...
	<nav class="navigation post-navigation" role="navigation">
		<h1 class="screen-reader-text"><?php _e( 'Post navigation', 'step' ); ?></h1>
		<div class="nav-links">
			<?php
...
function step_event_meta($cpost = null) {
	...
	if(!empty($date)){
		...
		echo "<span class='label'>".__('Event date', 'step').":</span>";
		echo "<time class='e-date'>".date('d.m.Y', strtotime($date))."</time>";
		echo "</div>";
	}
	if(!empty($time)){
		echo "<div class='em-field'>";
		echo "<span class='label'>".__('Time', 'step').":</span>";
		echo "<time class='e-time'>".apply_filters('step_the_title', $time)."</time>";
		echo "</div>";
	}
	if(!empty($addr)){
		echo "<div class='em-field'>";
		echo "<span class='label'>".__('Address', 'step').":</span>";
