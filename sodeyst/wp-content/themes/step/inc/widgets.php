...
class BB_Recent_Posts_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array(
			...
			'description' => __('List of recent posts with some styling', 'step')
		);
		$this->WP_Widget('widget_posts_recent', __('Recent Posts', 'step'), $widget_ops);
	}
	...
	?>
		<div class="rpw-title">
			<a href="<?php the_permalink();?>" title="<?php printf(esc_attr__('Permalink to %s', 'step'), the_title_attribute('echo=0')); ?>" rel="bookmark"><?php the_title();?></a>
		</div>	
	<?php
	}
	...
	function form($instance) {
		...
		$thumb_sizes = array(
			'none'           => __('Don\'t show', 'step'),
			'thumbnail'      => __('Square thumbnail', 'step'),
			'post-thumbnail' => __('Landscape thumnbail', 'step')
		);
	?>
				
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php _e('Title', 'step');?>:</label>
			<input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo $title; ?>"/>
		</p>
			
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('limit')); ?>"><?php _e('Limit', 'step'); ?>:</label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('date')); ?>"><?php _e('Display Date?', 'step'); ?></label>
			<input id="<?php echo $this->get_field_id('date'); ?>" name="<?php echo $this->get_field_name('date'); ?>" type="checkbox" value="1" <?php checked('1', $date); ?> />&nbsp;
		</p>
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('excerpt')); ?>"><?php _e('Display Excerpt?', 'step'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('length')); ?>"><?php _e('Excerpt Length', 'step'); ?>:</label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('thumb_size')); ?>"><?php _e('Thumbnail', 'step'); ?>:</label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('post_type'));?>"><?php _e('Choose the Post Type', 'step');?>:</label>

			<select class="widefat" id="<?php echo $this->get_field_id('post_type');?>" name="<?php echo $this->get_field_name('post_type');?>">
				<option value="0"><?php _e('Select post type', 'step');?></option>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('taxonomy'));?>"><?php _e('Choose the Taxonomy', 'step');?>:</label>

			<select class="widefat" id="<?php echo $this->get_field_id('taxonomy');?>" name="<?php echo $this->get_field_name('taxonomy');?>">
			<option value="0"><?php _e('Select taxonomy', 'step');?></option>
		...
		<p>
		<label for="<?php echo $this->get_field_id( 'slug' ); ?>"><?php _e('Term slug', 'step'); ?>:</label>
		...
		<small class="help"><?php _e('Copy paste a slug (or comma-separated list of several slugs) of the term', 'step'); ?> </small>
		</p>
...
class BB_Social_Links extends WP_Widget {
    ...
    function __construct() {
        $this->WP_Widget('widget_socila_links', __('Social Buttons', 'step'), array(
            'classname' => 'widget_socila_links',
            'description' => __('Social links menu with optional text', 'step'),
        ));
    }
