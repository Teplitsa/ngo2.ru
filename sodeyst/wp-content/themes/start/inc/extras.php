...
/** Default filters **/
add_filter( 'bb_the_content', 'wptexturize'        );
add_filter( 'bb_the_content', 'convert_smilies'    );
add_filter( 'bb_the_content', 'convert_chars'      );
add_filter( 'bb_the_content', 'wpautop'            );
add_filter( 'bb_the_content', 'shortcode_unautop'  );
add_filter( 'bb_the_content', 'do_shortcode' );

add_filter( 'bb_the_title', 'wptexturize'   );
add_filter( 'bb_the_title', 'convert_chars' );
add_filter( 'bb_the_title', 'trim'          );
...
/** more link */
function bb_get_excerpt_with_link($cpost) {
	$more = __('More', 'bb');
...
function bb_continue_reading_link() {
	$more = __('More', 'bb');
	return '&nbsp;<a href="'. esc_url( get_permalink() ) . '"><span class="meta-nav">'.$more.'</span></a>';
}

/** excerpt filters  */
add_filter( 'excerpt_more', 'bb_auto_excerpt_more' );
function bb_auto_excerpt_more( $more ) {
	return '&hellip;';
}

add_filter( 'excerpt_length', 'bb_custom_excerpt_length' );
function bb_custom_excerpt_length( $l ) {
	return 30;
}

/** inject */
add_filter( 'get_the_excerpt', 'bb_custom_excerpt_more' );
function bb_custom_excerpt_more( $output ) {
	global $post;
	
	if(is_singular() || is_search())
		return $output;
	
	$output .= bb_continue_reading_link();
	return $output;
}
 ...
 * Current URL
 **/
if(!function_exists('bb_current_url')){
function bb_current_url() {
...
function bb_get_posts_ids_from_query($query){
...
/** Copyright string */
function bb_dynamic_copyright($start_year, $name){
 ...
 **/
function bb_print_top_link(){

	if(!is_admin()):
 ?>	
	<div id="top-link">
		<a href="#top"><?php _e('On top', 'bb');?></a>
	</div>
	
<?php endif; 
}
add_action('wp_footer', 'bb_print_top_link');
...
add_action('wp_enqueue_scripts', 'bb_favicon', 1);
add_action('admin_enqueue_scripts', 'bb_favicon', 1);
add_action('login_enqueue_scripts', 'bb_favicon', 1);
 ...
 * 
 **/
add_filter( 'pre_wp_nav_menu', 'bb_pre_wp_nav_menu_social', 10, 2 );
function bb_pre_wp_nav_menu_social( $output, $args ) {
	...
	$supported_icons = apply_filters( 'bb_supported_social_icons', array(
		'app.net'            => 'fa-adn',
