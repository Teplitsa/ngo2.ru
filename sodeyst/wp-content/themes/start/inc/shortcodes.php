 ...
 **/
add_shortcode('bb_query', 'bb_query_screen');
function bb_query_screen($atts){
	global $wp_query;
	...
	$css = esc_attr($css);
	return  "<div class='bb-query {$css}'>{$out}</div>";
} 
...
function repeatable_blocks_screen($atts){
	...
	?>
		<section class="rb-item">
			...
			<?php if(!empty($title)): ?>
				<h3><?php echo apply_filters('bb_the_title', $title);?></h3>
			<?php endif; ?>
 ...
 **/
add_shortcode('my_togle', 'bb_my_toggle_screen');
function bb_my_toggle_screen($atts, $content = ''){
	...
	?>
		<section class="page-section <?php echo esc_attr($css);?>" <?php echo $style;?>>
			...
			<?php if(!empty($title)): ?>
				...
				<?php echo apply_filters('bb_the_title', $title);?>
				</h3>
			<?php endif; ?>
 ...
 * Markup shortcodes
 **/
add_shortcode('clear', 'bb_clear_screen');
function bb_clear_screen($atts){
...
add_shortcode('partners_gallery', 'bb_partners_gallery_screen');
function bb_partners_gallery_screen($atts){
