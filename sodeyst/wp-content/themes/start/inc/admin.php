...
add_filter('manage_posts_columns', 'bb_common_columns_names', 50, 2);
function bb_common_columns_names($columns, $post_type) {
...
add_action('manage_posts_custom_column', 'bb_common_columns_content', 2, 2);
function bb_common_columns_content($column_name, $post_id) {
...
add_filter('manage_pages_columns', 'bb_pages_columns_names', 50);
function bb_pages_columns_names($columns) {		
...
add_action('manage_pages_custom_column', 'bb_pages_columns_content', 2, 2);
function bb_pages_columns_content($column_name, $post_id) {
...
add_filter( "manage_edit-category_columns", 'bb_common_tax_columns_names', 10);
add_filter( "manage_edit-post_tag_columns", 'bb_common_tax_columns_names', 10);
function bb_common_tax_columns_names($columns){
...
add_filter( "manage_category_custom_column", 'bb_common_tax_columns_content', 10, 3);
add_filter( "manage_post_tag_custom_column", 'bb_common_tax_columns_content', 10, 3);
function bb_common_tax_columns_content($content, $column_name, $term_id){
...
add_action('admin_init', function(){
	foreach(get_post_types(array('public' => true), 'names') as $pt) {
		add_filter('manage_' . $pt . '_posts_columns', 'bb_clear_seo_columns', 100);
	}	
}, 100);

function bb_clear_seo_columns($columns){
...
/* Excerpt metabox */
add_action('add_meta_boxes', 'bb_correct_metaboxes', 2, 2);
function bb_correct_metaboxes($post_type, $post ){
	
	if(post_type_supports($post_type, 'excerpt')){
		remove_meta_box('postexcerpt', null, 'normal');
		
		$label = 'Аннотация';
		add_meta_box('bb_postexcerpt', $label, 'bb_excerpt_meta_box', null, 'normal', 'core');
	}
	//add_meta_box('postexcerpt', __('Excerpt'), 'post_excerpt_meta_box', null, 'normal', 'core');
}

function bb_excerpt_meta_box($post){
...
/* Menu Labeles */
add_action('admin_menu', 'bb_admin_menu_labels');
function bb_admin_menu_labels(){ /* change adming menu labels */
    global $menu, $submenu;
    ...
    foreach($menu as $order => $item){
        ...
        if($item[2] == 'edit.php?post_type=acf-field-group'){
            $menu[$order][0] = __('Custom fields', 'bb');
            break;
        }
    }   
