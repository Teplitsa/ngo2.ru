...
add_action('widgets_init', 'bb_custom_widgets', 11);
function bb_custom_widgets(){
...
class BB_RSS_Widget extends WP_Widget {
    ...
    function widget($args, $instance) {
		...
		$link_txt = apply_filters('bb_the_title', $instance['link_txt']);
...
class BB_Recent_Posts_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array(
			...
			'description' => __('List of recent posts with some styling', 'bb')
		);
		$this->WP_Widget('widget_posts_recent', __('Recent Posts', 'bb'), $widget_ops);
	}
	...
	function widget($args, $instance) {
		...
		<div class="rpw-block<?php if($has_thumb) echo ' has-thumb';?>">
			...
			<?php
				...
				$sep = apply_filters('rpw_post_meta_separator', bb_get_sep(), $post);
	...
	?>
		<div class="rpw-title">
			<a href="<?php the_permalink();?>" title="<?php printf(esc_attr__('Permalink to %s', 'bb'), the_title_attribute('echo=0')); ?>" rel="bookmark"><?php the_title();?></a>
		</div>	
	<?php
	}
	...
	function form($instance) {
		...
		$thumb_sizes = array(
			'none'           => __('Don\'t show', 'bb'),
			'thumbnail'      => __('Square thumbnail', 'bb'),
			'post-thumbnail' => __('Landscape thumnbail', 'bb')
		);
	?>
				
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php _e('Title', 'bb');?>:</label>
			<input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo $title; ?>"/>
		</p>
			
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('limit')); ?>"><?php _e('Limit', 'bb'); ?>:</label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('date')); ?>"><?php _e('Display Date?', 'bb'); ?></label>
			<input id="<?php echo $this->get_field_id('date'); ?>" name="<?php echo $this->get_field_name('date'); ?>" type="checkbox" value="1" <?php checked('1', $date); ?> />&nbsp;
		</p>
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('excerpt')); ?>"><?php _e('Display Excerpt?', 'bb'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('length')); ?>"><?php _e('Excerpt Length', 'bb'); ?>:</label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('thumb_size')); ?>"><?php _e('Thumbnail', 'bb'); ?>:</label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('post_type'));?>"><?php _e('Choose the Post Type', 'bb');?>:</label>

			<select class="widefat" id="<?php echo $this->get_field_id('post_type');?>" name="<?php echo $this->get_field_name('post_type');?>">
				<option value="0"><?php _e('Select post type', 'bb');?></option>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('taxonomy'));?>"><?php _e('Choose the Taxonomy', 'bb');?>:</label>

			<select class="widefat" id="<?php echo $this->get_field_id('taxonomy');?>" name="<?php echo $this->get_field_name('taxonomy');?>">
			<option value="0"><?php _e('Select taxonomy', 'bb');?></option>
		...
		<p>
		<label for="<?php echo $this->get_field_id( 'slug' ); ?>"><?php _e('Term slug', 'bb'); ?>:</label>
		...
		<small class="help"><?php _e('Copy paste a slug (or comma-separated list of several slugs) of the term', 'bb'); ?> </small>
		</p>
...
class BB_Featured_Post_Widget extends WP_Widget {
	...
	?>
		<div class="fw-item">
			...
			<div class="entry-summary"><?php echo apply_filters('bb_the_content', apl_get_excerpt_with_link($cpost));?></div>
		</div>
...
class BB_Social_Links extends WP_Widget {
    ...
    function __construct() {
        $this->WP_Widget('widget_socila_links', __('Social Buttons', 'bb'), array(
            'classname' => 'widget_socila_links',
            'description' => __('Social links menu with optional text', 'bb'),
        ));
    }
