...
<?php if ( empty( $entries ) ) { ?>
    <tr><td colspan="<?php echo count($form_cols) ?>"><?php echo $atts['no_entries']; ?></td></tr>
<?php
}else{
    $class = 'odd';
...
<?php       }
            ...
            if ( $atts['edit_link'] ) { ?>
        <td><?php if ( FrmProEntriesHelper::user_can_edit($entry, $form) ) { ?><a href="<?php echo esc_url(add_query_arg( array( 'frm_action' => 'edit', 'entry' => $entry->id), $permalink) . $anchor)  ?>"><?php echo $atts['edit_link']; ?></a><?php } ?></td>
<?php       }
            if ( $atts['delete_link'] ) { ?>
