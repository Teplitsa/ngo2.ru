 ...
 **/
add_shortcode('memo_query', 'memo_query_screen');
function memo_query_screen($atts){
	global $wp_query;
	...
	// On singular pages page=2 qv detects paging:
	...
	$css = esc_attr($css);
	return  "<div class='step-query {$css}'>{$out}</div>";
} 
...
function repeatable_blocks_screen($atts){
	...
	?>
		<section class="rb-item">
			...
			<?php if(!empty($title)): ?>
				<h3><?php echo apply_filters('memo_the_title', $title);?></h3>
			<?php endif; ?>
 ...
 **/
add_shortcode('my_togle', 'memo_my_toggle_screen');
function memo_my_toggle_screen($atts, $content = ''){
	...
	?>
		<section class="page-section <?php echo esc_attr($css);?>" <?php echo $style;?>>
			...
			<?php if(!empty($title)): ?>
				...
				<?php echo apply_filters('memo_the_title', $title);?>
				</h3>
			<?php endif; ?>
 ...
 * Markup shortcodes
 **/
add_shortcode('clear', 'memo_clear_screen');
function memo_clear_screen($atts){
...
add_shortcode('partners_gallery', 'memo_partners_gallery_screen');
function memo_partners_gallery_screen($atts){
...
?>
    ...
    <?php
		foreach($query->posts as $item):
			...
			$cat = memo_get_partner_type($item);
        ?>
