...
add_filter('manage_posts_columns', 'memo_common_columns_names', 50, 2);
function memo_common_columns_names($columns, $post_type) {
		
	if(in_array($post_type, array('post', 'book', 'document', 'attachment'))){
...
add_action('manage_posts_custom_column', 'memo_common_columns_content', 2, 2);
function memo_common_columns_content($column_name, $post_id) {
...
add_filter('manage_pages_columns', 'memo_pages_columns_names', 50);
function memo_pages_columns_names($columns) {		
...
add_action('manage_pages_custom_column', 'memo_pages_columns_content', 2, 2);
function memo_pages_columns_content($column_name, $post_id) {
...
//add_filter( "manage_edit-category_columns", 'memo_common_tax_columns_names', 10);
//add_filter( "manage_edit-post_tag_columns", 'memo_common_tax_columns_names', 10);
function memo_common_tax_columns_names($columns){
...
//add_filter( "manage_category_custom_column", 'memo_common_tax_columns_content', 10, 3);
//add_filter( "manage_post_tag_custom_column", 'memo_common_tax_columns_content', 10, 3);
function memo_common_tax_columns_content($content, $column_name, $term_id){
...
add_action('admin_init', function(){
	foreach(get_post_types(array('public' => true), 'names') as $pt) {
		add_filter('manage_' . $pt . '_posts_columns', 'memo_clear_seo_columns', 100);
	}	
}, 100);

function memo_clear_seo_columns($columns){
...
/* Excerpt metabox */
add_action('add_meta_boxes', 'memo_correct_metaboxes', 2, 2);
function memo_correct_metaboxes($post_type, $post ){
	
	if(post_type_supports($post_type, 'excerpt')){
		remove_meta_box('postexcerpt', null, 'normal');
		
		$label = 'Аннотация';
		add_meta_box('memo_postexcerpt', $label, 'memo_excerpt_meta_box', null, 'normal', 'core');
	}
	//add_meta_box('postexcerpt', __('Excerpt'), 'post_excerpt_meta_box', null, 'normal', 'core');
}

function memo_excerpt_meta_box($post){
